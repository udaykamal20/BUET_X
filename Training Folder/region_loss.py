...
def build_targets(pred_boxes, target, anchors, num_anchors, nH, nW, noobject_scale, object_scale, sil_thresh, seen):
    ...
    for b in range(nB):
        for t in range(50):
            ...
            gw = target[b][t*5+3]*nW
            gh = target[b][t*5+4]*nH
            gt_box = [0.0, 0.0, gw, gh]
            for n in range(nA):
                aw = anchors[anchor_step*n]
                ah = anchors[anchor_step*n+1]
                anchor_box = [0.0, 0.0, aw, ah]
...
def get_giou_loss(output,label,anchors,num_anchors,anchor_step):

    all_giou = torch.zeros(1)
    all_giou = Variable(all_giou.cuda())
    batch = output.size(0)
    h = output.size(2)
    w = output.size(3)
    ...
    for b in range(batch):
        det_confs_inb = det_confs[b*sz_hwa:(b+1)*sz_hwa]
        xs_inb = xs[b*sz_hwa:(b+1)*sz_hwa]
        ys_inb = ys[b*sz_hwa:(b+1)*sz_hwa]
        ws_inb = ws[b*sz_hwa:(b+1)*sz_hwa]
        hs_inb = hs[b*sz_hwa:(b+1)*sz_hwa]      
        ind = torch.argmax(det_confs_inb)
        bcx = xs_inb[ind]
        bcy = ys_inb[ind]
        bw = ws_inb[ind]
        bh = hs_inb[ind]
        ...
        all_giou += bbox_giou(box, label[b][1:5].cuda().float(), x1y1x2y2=False)
    return all_giou
...
#        all_giou = get_giou_loss(output,target,self.anchors,self.num_anchors,self.anchor_step)
...
#        print('%d: nGT %d, recall %d,  loss: x %f, y %f, w %f, h %f, conf %f, total %f' % (self.seen, nGT, nCorrect, loss_x.data, loss_y.data, loss_w.data, loss_h.data, loss_conf.data,  loss.data))

#        return loss, [nCorrect, loss_x.data, loss_y.data, loss_w.data, loss_h.data, loss_conf.data,  loss.data]
        return loss, [nCorrect, loss_conf.data,  loss.data]